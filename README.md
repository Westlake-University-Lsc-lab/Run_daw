...
Run DAQ and configure the pulse generator automatically.
...
#### Configure LED pulse generator

Type the following command in the terminal as an example:
...
This script just transmits the specific parameter to the Pulse Generator; the syntax is in 'ConfigPulseGen.py'.
You can change and edit depending on your own requirements.  
...
'Comb' --- Whether to combine the voltage from the 2 channels to output Ch1?  
...
```
=======
This script is to generate a config file before running DAW_Demo. The input file is 'DAW_Config.txt' automatically.  
The output file is 'configure_new.txt'.  
Parameter explanation:  
'trig_style' --- str type, 'ext' or 'self', trigger mode
'rec_len' --- int type, min record length of a raw waveform. self --> rec_len = 5, ext --> rec_len = 25.
'acq_time' --- int type, data acquisition time length, unit by second  
'threshold.json' --- int type,self-trigger threshold.json file, only efficient under 'self' trigger mode  
~'file_name' --- str type, rawdata file tag ~
>>>>>>> 392af227df69ba6d315b0664e93b3524b876d5b0
...
The <runtype> can be an input, or can be a default value, like run5_Ar, run6_Xe;
The <run_id> will be generated automatically, like 00001, 00099, ...
...
The runconfiginfo.py will generate a runinfo.json file, and copy the original configure_new.txt to the directory as a new name config.cfg
...
It needs three JSON files as input, `dec_params.json`, `threshold.json`, `mapping.json`, and needs some parameters like:
...
This should be under the 'daq' account and run daq.
And you should press 's' on your keyboard to start data recording, press
'q' to quit the DAQ process, it will be stopped after 'ACQ_TIME' seconds.
...
You need to give some input parameters, like `WRITECONFIG_TRIGGER`, `DAQ_ACQ_TIME`, `RUNINFO_COMMENT`, the `WRITECONFIG_TRIGGER=self` by default, if you want to take LED external data, you shold give it `ext`trigger mode. `DAQ_ACQ_TIME` is the data take time length for a run; after a run, it will start a new run automatically. `RUNINFO_COMMENT` will store the feature about this run, like `LED Calibration, Kr Calibration, Xe Overnight Run` or something that can explain the data.
...
This is just a tricky script to auto-configure the LED and then run DAQ, which should be customized.  
the below is just an example script to do this
...
This is just a simple example, the LED pulse generator and the DAQ configure file
